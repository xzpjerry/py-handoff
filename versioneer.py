...
def render_pep440_xzp(pieces):
    ...
    if tag:
        if parsed_working < parsed_tag:
            if parsed_working.base_version == parsed_tag.base_version:
                ...
                raise Exception(
                    "Developing version can not go back in time: %s < %s" % working, tag
                )
        elif parsed_working == parsed_tag:
            if pieces["distance"] or pieces["dirty"]:
                rendered += ".post1.dev%d" % pieces["distance"]
    ...
    if (
        ...
        or tracking_branch.startswith("origin/hotfix/")
    ):
        return rendered
    rendered += ".g%s" % pieces["short"]
    return rendered
...
def is_contain_dev_dependency():
    for r in parse_requirements("./requirements.txt", session=PipSession()):
        for spec in r.specifier._specs:
            if spec.operator != "==":
                raise Exception("Dependency not pinned in requirements.txt")
            if not is_canonical(spec.version):
                continue
            if (
                re.match(
                    r"^([1-9][0-9]*!)?(0|[1-9][0-9]*)(\.(0|[1-9][0-9]*))*(\.post(0|[1-9][0-9]*))?$",
                    spec.version,
                )
                is None
            ):
